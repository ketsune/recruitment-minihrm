...
const ApproveTable = ({ data, onSearchChange, sortKey, direction, handleSort, onConfirm, checkStatus, reject, changeStatus, clearStatus, setApproveDate, setApproveTime }) => (
  <div>
    ...
    <Table striped sortable selectable celled style={{ overflowX: 'auto' }}>
      <Table.Header>
        <Table.Row>
          <Table.HeaderCell sorted={sortKey === 'nfirstName' ? direction : null} onClick={() => handleSort('firstName')}>Name</Table.HeaderCell>
          ...
          <Table.HeaderCell colSpan="4">
            <Form onSubmit={onConfirm}>
              <Form.Group floated="left">
                <Field name="date" as={Form.Input} component={Input} label="Data" placeholder="Ex. 2018-07-23" type="date" onChange={(event, value) => setApproveDate(value)} />
