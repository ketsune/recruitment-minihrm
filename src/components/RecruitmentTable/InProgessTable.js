...
const row = (item, { checkStatus, reject, selectStatus, changeStatus }) => (
  <Table.Row key={item.citizenId}>
    ...
    <Table.Cell><Checkbox name='accept' checked={checkStatus[item.citizenId] === 'Pass' ? true : false} onChange={() => changeStatus(item.citizenId, 'Pass')} /></Table.Cell>
    {reject && <Table.Cell><Checkbox name='reject' checked={checkStatus[item.citizenId] === 'Fail' ? true : false} onChange={() => changeStatus(item.citizenId, 'Fail')} /></Table.Cell>}
  </Table.Row>
);

const InProgressTable = ({ data, onSearchChange, sortKey, direction, handleSort, onConfirm, checkStatus, reject, selectStatus, changeStatus, clearStatus }) => (
  <div>
